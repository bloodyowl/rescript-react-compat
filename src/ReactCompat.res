type rec component<'state, 'initialState, 'action> = {
...
and self<'state, 'action> = {
  handle: 'payload. (('payload, self<'state, 'action>) => unit, 'payload) => unit,
  state: 'state,
  send: 'action => unit,
...
@ocaml.doc(" This is not exposed, only used internally so that useReducer can
...
let useRecordApi = componentSpec => {
  ...
  let ({state, sideEffects}, send) = React.useReducer((fullState, action) =>
    @ocaml.doc("
         ...
         ")
    switch componentSpec.reducer(action, fullState.state.contents) {
    ...
    | SideEffects(sideEffect) => {
        ...fullState,
        sideEffects: ref(Js.Array.concat(fullState.sideEffects.contents, [sideEffect])),
      }
  ...
  @ocaml.doc(" This is the temp self for willReceiveProps ")
  let rec self = {
    handle: (fn, payload) => fn(payload, self),
    send: send,
    state: state.contents,
    onUnmount: sideEffect => Js.Array.push(sideEffect, unmountSideEffects.current)->ignore,
  }
  ...
  @ocaml.doc(" There might be some potential issues with willReceiveProps,
  ...
  let _mountUnmountEffect = React.useEffect0(() => {
    ...
    Some(
      () => {
        ...
        componentSpec.willUnmount(upToDateSelf.current)
      },
    )
  })

  let _didUpdateEffect = React.useEffect(() => {
  ...
  @ocaml.doc(" Because sideEffects are only added through a **new** ref,
      we can use the ref itself as the dependency. This way the
      effect doesn't re-run after a cleanup.
  ")
  React.useEffect1(() => {
    if Js.Array.length(sideEffects.contents) > 0 {
  ...
  let mostRecentAllowedRender = React.useRef(React.useMemo0(() => componentSpec.render(self)))

  upToDateSelf.current = self
...
module Defaults = {
  ...
  let willReceivePropsDefault: self<'state, 'action> => 'state = ({state}) => state
  let renderDefault = _self => React.string("RenderNotImplemented")
  let initialStateDefault = () => ()
  let reducerDefault: ('action, 'state) => update<'state, 'action> = (_action, _state) => NoUpdate
}
